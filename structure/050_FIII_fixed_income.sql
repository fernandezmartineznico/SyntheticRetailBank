...
-- │
-- ├─ STORED PROCEDURES (1):
-- │  └─ FIII_CLEANUP_STAGE_KEEP_LAST_N - Generic stage cleanup utility
-- │
-- └─ TASKS (2 - All Serverless: 1 load + 1 cleanup):
--    ├─ FIII_LOAD_TRADES_TASK - Automated CSV loading
--    └─ FIII_RAW_TASK_CLEANUP_STAGE_AFTER_LOAD_TRADES - Stage cleanup
--
-- DATA FLOW:
...
-- ------------------------------------------------------------
-- FIII_CLEANUP_STAGE_KEEP_LAST_N - Generic Stage Cleanup Procedure
-- ------------------------------------------------------------
-- Removes oldest files from any stage in FII_RAW_001 schema, keeping only
-- the N most recently modified files. Useful for managing stage storage
-- costs and implementing data retention policies.
--
-- Parameters:
--   STAGE_NAME: Stage name without @ prefix (e.g., 'FIII_RAW_STAGE_TRADES')
--   KEEP_N: Number of most recent files to retain
--
-- Returns: Summary string with deletion statistics
--
-- Usage Example:
--   CALL FIII_CLEANUP_STAGE_KEEP_LAST_N('FIII_RAW_STAGE_TRADES', 5);

CREATE OR REPLACE PROCEDURE FIII_CLEANUP_STAGE_KEEP_LAST_N (
    STAGE_NAME STRING,
    KEEP_N     NUMBER
)
RETURNS STRING
...
-- Cleanup task for fixed income trades stage
CREATE OR REPLACE TASK FIII_RAW_TASK_CLEANUP_STAGE_AFTER_LOAD_TRADES
    USER_TASK_MANAGED_INITIAL_WAREHOUSE_SIZE = 'XSMALL'
    COMMENT = 'Automated stage cleanup after fixed income trade data load. Keeps last 5 files to manage storage costs.'
    AFTER FIII_LOAD_TRADES_TASK
AS
    CALL FIII_CLEANUP_STAGE_KEEP_LAST_N('FIII_RAW_STAGE_TRADES', 5);
